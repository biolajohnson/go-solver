...
class AlphaBetaPlayer():
	...
	def get_input(self, go, piece_type, board, prev_board):
		go_copy = deepcopy(go) 
		board_state = Board(board, piece_type, None, self.move_number) 
		start = time()
		...
		if best_move != "PASS" and not go_copy.valid_place_check(best_move[0], best_move[1], piece_type, test_check=True):
			return self.move_number + 1, "PASS"
		end = time()
		print("move: {} it took {:.2f} secs".format(best_move, end - start))
		return self.move_number + 1, best_move
...
class Board():
	def __init__(self, board, player, prev, n_moves):
		self.board = board
	...
	def move(self, action, piece_type):
		board = self.board
		if action == "PASS":
			return Board(board, self.opp, action, self.n_moves + 1)
	...
	def utility(self, player):
		eval = self.evaluation(player)
		player_score = self.score(player)
		opp_score = self.score(3 - player)
		if player == 1:
			...
			for j in range(0, 5):
				if self.board[i][j] != 0:
					...
					if self.board[i][j] == player:
						...
						opp_count += 1
		edge_piece =  player_edge_piece	- opp_edge_piece
...
def is_new_game(piece_type, board):
	if piece_type == 1:
		return is_first_move(board)
	black_piece = 0
	for i in range(0, 5):
		for j in range(0, 5):
			if (board[i][j] != 0):
...
def play():
	...
	new_game = is_new_game(piece_type, board)
	if new_game:
		write_move(1)
	...
	move_no = read_move()
	player = AlphaBetaPlayer(move_no)
	print(move_no)
	move_number, action = player.get_input(go, piece_type, board, previous_board)
	writeOutput(action)
	print(action)
	write_move(move_number)
...
def write_move(move, path="/Users/biolajohnson/Documents/assignments/561/Homework2/test/play2.txt"):
	with open(path, 'w') as f:
		f.write(str(move))

def read_move(path="/Users/biolajohnson/Documents/assignments/561/Homework2/test/play2.txt"):
	with open(path, 'r') as f:
		move_number = f.readline()
